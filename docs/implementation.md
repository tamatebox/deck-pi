...
| 7 | **Absence of a complaint read as evidence** | break the thing on purpose and confirm the check complains | a linter whose error went to stderr and whose silence was read as a pass; a symmetric null test; a race harness reaching `Overrun` 8.6M times and detecting nothing; a guard whose enforcement depended on **linkage** — an integration test that never touches the crate gets no `#[global_allocator]`, so every `assert_no_alloc` in it silently passes; and the stick's udev rule, which parsed, passed `udevadm verify`, was read by udev, and matched nothing — found only by instrumenting the guards with a marker property and watching how far it got; and the pitch fader's bench probe, where `cat /dev/ttyACM0` blocks for ever with no output and no error, because the firmware waits on DTR and that open does not deliver it — **a silent terminal is precisely what a dead fader would also produce**, so the instrument could not tell a working control from an unwired one, and the first reading nearly went down as a failure. Cleared by proving the channel on its own terms — `CLOCAL` set and DTR raised explicitly — before believing anything the channel did not say; the recipe is in `cdj-200.md` |
